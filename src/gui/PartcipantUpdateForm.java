...
public class PartcipantUpdateForm extends ScrollPane
{
    ...
    public PartcipantUpdateForm(int cosmoID, StaffAccount loggedInUser)
    {
        ...
        mainBox.getChildren().addAll(createHeader(), saveMessage,
                kinInfoAreaHeader, createKinInfoArea(), line2,
                caregiverInfoAreaHeader, createCaregiverInfoArea(), line3,
     ...
     * 
     * @return the HBox containing the heading label and the save button.
     * @author Breanna Wilson cst215 Steven Palchinski cst209
     */
        ...
        if ( loggedInUser instanceof MedicalAdministrator )
        {
            printPreviewBtn
                    .setOnAction(event -> {
                        ...
                        previewGUI.PrintPreviewUpdateFormGUIConstruct(
                                previewStage, cosmoID, loggedInUser);
    ...
    /**
     * Purpose: Saves the information in the form to the database. If the
     * information given is invalid, the information will not be saved. A
     * message will display for both success and failure.
     * 
    ...
    private void saveInfo()
    {
        ...
        errorMsg += controller.saveKinInformation(kinFirstNameTxt.getText(),
                kinLastNameTxt.getText(), kinAddressTxt.getText(),
                kinCityTxt.getText(), kinPostalCodeTxt.getText(),
                kinHomePhoneTxt.getText(), kinWorkPhoneTxt.getText());

        errorMsg += controller.saveCaregiverInformation(
                caregiverFirstNameTxt.getText(),
                caregiverLastNameTxt.getText(), caregiverAddressTxt.getText(),
                caregiverCityTxt.getText(), caregiverPostalCodeTxt.getText(),
                caregiverHomePhoneTxt.getText(),
                caregiverWorkPhoneTxt.getText());

        errorMsg += controller.saveEmergencyContactInformation(
                emergencyContactFirstName.getText(),
                emergencyContactLastName.getText(),
                emergencyContactPhone.getText());
