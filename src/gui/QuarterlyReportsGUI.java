...
/**
 * The interface for previewing the querterly reports based on the dates
 * selected.
 * 
...
public class QuarterlyReportsGUI extends Application
{
    ...
    public QuarterlyReportsGUI(Stage stage)
    {
        ...
        // When the ending date has been set or changed, update the column
        // headers
        endDate.setOnAction(event -> {
            if ( endDate.getValue() != null )
            {
                olderAdultNeedsTVCont.setTotalAsOfColumn("Total As of: "
                        + formatDate(endDate.getValue()));
                personalCareTVCont.setTotalAsOfColumn("Total As of: "
                        + formatDate(endDate.getValue()));
                olderAdultNeedsTVCont.setTotalForLastYearColumn("Total For "
                        + (endDate.getValue().getYear() - 1));
                personalCareTVCont.setTotalForLastYearColumn("Total For "
                        + (endDate.getValue().getYear() - 1));
            }
        });
    }
    ...
    {
        // Nothing is necessary to do here, as it is done in the constructor
        // instead
    }
    ...
    /**
     * Creates the VBox containing the header label and tableview for the adult
     * needs table.
     * 
    ...
    private VBox createOlderAdultNeedsArea()
    {
        ...
        vbox.getChildren().addAll(areaLbl,
                olderAdultNeedsTVCont.adultNeedsTable);

        return vbox;
    }

    /**
     * Creates the label and tableview for the personal health care area.
     * 
     * @return The VBox containing the label and tableview for participant
    ...
    private VBox createPersonalHealthCareArea()
    {
        ...
        vbox.getChildren()
                .addAll(areaLbl, personalCareTVCont.personalCareTable);

        return vbox;
    }

    /**
     * Creates the label and tableview for the reactive care area.
     * 
     * @return The VBox containing the label and tableview for the reactive care
     *         area
     * 
     * @author Breanna Wilson, Jon Froese
     */
    private VBox createReactiveCareArea()
    {
        ...
        vbox.getChildren()
                .addAll(areaLbl, reactiveCareTVCont.reactiveCareTable);
     ...
     * 
     * @return The VBox containing the label and tableview for the health
     *         promotion area
     * 
     * @author Breanna Wilson, Jon Froese
     */
    private VBox createHealthPromotionArea()
    {
        ...
        Label areaLbl = new Label(
                "Health Promotion Activities and Health-Related Supplies"
    ...
    private void exportReport()
    {
        ...
        {
            PopUpMessage msg = new PopUpMessage("Start Date must be set.",
                    stage);
        ...
        {
            QuarterlyReportExportHelper helper = new QuarterlyReportExportHelper(
                    personalCareTVCont.personalCareData,
                    reactiveCareTVCont.reactiveCareData,
                    olderAdultNeedsTVCont.adultData,
                    formatDate(endDate.getValue()), endDate.getValue()
                            .getYear() - 1);
            helper.exportToCSV();

            PopUpMessage msg = new PopUpMessage(
                    "Quarterly Report has been exported.", stage);
            msg.stage.showAndWait();
        }
