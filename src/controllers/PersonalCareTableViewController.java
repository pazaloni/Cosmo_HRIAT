...
public class PersonalCareTableViewController 
{
	...
	private void queryPersonalCareData()
	{
		...
		rs = db.select("COUNT(depoInjection)", "Participant", "depoInjection = TRUE", "");
		...
		rs = db.select("COUNT(b12Injection)", "Participant", "b12Injection = TRUE", "");
		...
		rs = db.select("COUNT(suprapubicCatheter)", "Participant", "suprapubicCatheter = TRUE", "");
		...
		rs = db.select("COUNT(tracheotomy)", "Participant", "tracheotomy = TRUE", "");
		...
		rs = db.select("COUNT(category)", "Allergies", "category = \"Environmental\"", "");
		...
		rs = db.select("COUNT(conditionName)", "Conditions", "conditionName = \"Diabetes\"" + 
		...
		rs = db.select("COUNT(dietProvided)", "Participant", "dietProvided = TRUE", "");
		...
		rs = db.select("COUNT(transfersLifts)", "Participant", "transfersLifts LIKE \"full\"", "");
		...
		rs = db.select("COUNT(transfersLifts)", "Participant", "transfersLifts LIKE \"sit\" OR"
		...
		rs = db.select("COUNT(assistanceRequired)", "Participant", "assistanceRequired LIKE \"2\"", "");
		...
		rs = db.select("COUNT(assistanceRequired)", "Participant", "assistanceRequired LIKE \"1\"", "");
	...
	public void initialize()
	{
        ...
        personalCareTable.getColumns().addAll(typeColumn, supportsColumn, totalAsOfColumn,
