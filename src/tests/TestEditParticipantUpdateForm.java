...
{
    int cosmoID = 7542;
    ParticipantUpdateFormController controller = new ParticipantUpdateFormController(
            cosmoID);
    ...
    /**
     * Purpose: Resets the error message for each test, to that one from the
     * previous test does not affect the current one.
     * 
     * @throws Exception
     */
    @Before
    ...
    /**
     * Purpose: Test that when invalid kin information is given for a
     * participant, it fails the checks for validation.
     */
    @Test
    public void testInvalidKinInformation()
    {
        ...
        errorMsg = controller.saveKinInformation(iKinFirstName, iKinLastName,
                iKinAddress, iKinCity, iKinPostalCode, iKinHomePhone,
                iKinWorkPhone);
        ...
        errorMsg = controller.saveKinInformation("", "", kinAddress, kinCity,
                kinPostalCode, kinHomePhone, kinWorkPhone);
        // there should be an error message
        assertFalse(errorMsg.equals(""));
    ...
    /**
     * Purpose: Test that when valid caregiver information is given, it passes
     * validation checks to go into the database.
     */
    @Test
    public void testValidCaregiverInformation()
    {
        // try to save using valid information for all fields
        errorMsg = controller.saveCaregiverInformation(caregiverFirstName,
                caregiverLastName, caregiverAddress, caregiverCity,
    ...
    /**
     * Purpose: Test that when invalid caregiver information is given, it does
     * not pass the validation checks for saving.
     */
    @Test
    public void testInvalidCaregiverInformation()
    {
        // try to save using invalid caregiver information for all fields
        errorMsg = controller.saveCaregiverInformation(iCaregiverFirstName,
                iCaregiverLastName, iCaregiverAddress, iCaregiverCity,
                iCaregiverPostalCode, iCaregiverHomePhone, iCaregiverWorkPhone);
        // there should be an error message
        assertFalse(errorMsg.equals(""));
        ...
        errorMsg = controller.saveCaregiverInformation("", "",
                caregiverAddress, caregiverCity, caregiverPostalCode,
                caregiverHomePhone, caregiverWorkPhone);
        // there should be an error message
        assertFalse(errorMsg.equals(""));
    }

    /**
     * Purpose: Test that when valid emergency contact information is given, it
     * passes the validation checks for saving
     */
    @Test
    ...
    public void testInvalidEmergencyContactInformation()
    {
        ...
        errorMsg = controller.saveEmergencyContactInformation(
                iEmergencyContactFirstName, iEmergencyContactLastName,
                iEmergencyContactPhone);
        // there should be an error message
        assertFalse(errorMsg.equals(""));
    ...
    /**
     * Purpose: Test the postal code formatter, with both formatted and
     * unformatted valid and invalid postal codes
     */
    @Test
